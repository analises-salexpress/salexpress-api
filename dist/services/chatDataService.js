...
exports.toolGetTopClients = toolGetTopClients;
exports.toolGetClientDetail = toolGetClientDetail;
exports.toolGetChurnRisk = toolGetChurnRisk;
exports.toolGetGrowthClients = toolGetGrowthClients;
exports.toolGetRouteAnalysis = toolGetRouteAnalysis;
exports.toolSearchClients = toolSearchClients;
const prisma_1 = require("../db/prisma");
function cutoff(monthsBack) {
    const now = new Date();
...
async function toolGetOverview({ months_back = 6 }) {
    ...
    const [monthlyRows, clientMap, allRoutes] = await Promise.all([
        ...
        buildClientMap(),
        prisma_1.prisma.biAllRoute.findMany({ orderBy: { totalRevenue: 'desc' }, take: 12 }),
    ]);
    ...
    for (const r of monthlyRows) {
        ...
        if (!trendMap.has(key))
            trendMap.set(key, { billing: 0, clients: new Set(), ctrc: 0 });
        const t = trendMap.get(key);
        t.billing += r.billing;
        t.clients.add(r.clientCnpj);
        t.ctrc += r.deliveriesCount;
        const c = clientMap.get(r.clientCnpj);
        const seg = c?.segment ?? 'Não classificado';
        const st = c?.state ?? 'N/A';
        const cv = c?.curve ?? 'N/A';
        for (const [map, key2] of [[segMap, seg], [stateMap, st], [curveMap, cv]]) {
            if (!map.has(key2))
                map.set(key2, { billing: 0, clients: new Set() });
            const e = map.get(key2);
            e.billing += r.billing;
            e.clients.add(r.clientCnpj);
        }
    }
...
async function toolGetTopClients({ limit = 20, months_back = 3, segment, state, curve, }) {
    const cut = cutoff(months_back);
    const now = new Date();
    ...
    for (const r of rows) {
        if (!billingMap.has(r.clientCnpj))
            billingMap.set(r.clientCnpj, { billing: 0, ctrc: 0, months: 0 });
        const e = billingMap.get(r.clientCnpj);
        e.billing += r.billing;
        e.ctrc += r.deliveriesCount;
        e.months++;
    }
    const where = { cnpj: { in: Array.from(billingMap.keys()) } };
    if (segment)
    ...
    if (curve)
        where.curve = curve;
    const clients = await prisma_1.prisma.biClient.findMany({ where });
    const fmt = (n) => Math.round(n * 100) / 100;
    return {
...
async function toolGetClientDetail({ cnpj, name }) {
    let client = null;
    if (cnpj) {
        client = await prisma_1.prisma.biClient.findUnique({ where: { cnpj } });
    }
    else if (name) {
        const res = await prisma_1.prisma.biClient.findMany({
    ...
    if (!client)
        return { error: 'Cliente não encontrado. Tente buscar por nome parcial ou CNPJ exato.' };
    const now = new Date();
    ...
    const fmt = (n) => Math.round(n * 100) / 100;
    const completed = monthlyRows.filter((r) => !(r.year === now.getFullYear() && r.month === now.getMonth() + 1));
...
async function toolGetChurnRisk({ threshold_pct = 20, limit = 25, }) {
    const now = new Date();
    const rows = await prisma_1.prisma.biClientMonthly.findMany({
        where: {
            NOT: { year: now.getFullYear(), month: now.getMonth() + 1 },
    ...
    for (const r of rows) {
        if (!byClient.has(r.clientCnpj))
            byClient.set(r.clientCnpj, []);
    ...
    for (const [cnpj, months] of byClient) {
        ...
        if (changePct <= -threshold_pct)
            candidates.push({ cnpj, avgRecent, avgPrev, changePct });
    }
...
async function toolGetGrowthClients({ limit = 20 }) {
    const now = new Date();
    const rows = await prisma_1.prisma.biClientMonthly.findMany({
    ...
    for (const [cnpj, months] of byClient) {
        ...
        const changePct = ((avgRecent - avgPrev) / avgPrev) * 100;
        if (changePct > 0)
            candidates.push({ cnpj, avgRecent, avgPrev, changePct, absoluteGrowth: avgRecent - avgPrev });
    }
...
async function toolGetRouteAnalysis() {
    const [allRoutes, clientRoutes] = await Promise.all([
        prisma_1.prisma.biAllRoute.findMany({ orderBy: { totalRevenue: 'desc' } }),
        prisma_1.prisma.biClientRoute.findMany({ select: { region: true } }),
    ]);
...
async function toolSearchClients({ query, limit = 10 }) {
    const clients = await prisma_1.prisma.biClient.findMany({
        where: {
            OR: [
