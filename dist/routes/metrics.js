...
router.get('/dashboard', async (req, res) => {
    ...
    const allCards = await prisma_1.prisma.kanbanCard.findMany({
        where: req.user.role === client_1.Role.VENDOR ? { assignedToId: req.user.userId } : {},
    ...
    const activeCards = await prisma_1.prisma.kanbanCard.findMany({
        where: {
            status: { in: ['IDENTIFIED', 'CONTACTED', 'NEGOTIATING'] },
            ...(req.user.role === client_1.Role.VENDOR ? { assignedToId: req.user.userId } : {}),
